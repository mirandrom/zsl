...
def main(cfg: TrainConfig) -> None:
    ...
    with Trainer(
        ...
        if cfg.compile is not None:
            # HACK: for a triton bug - https://github.com/pytorch/pytorch/issues/124565 # ZSL
...
if __name__ == "__main__":
    ...
    # set ZSL env vars
    os.environ['ZSL_DIR_OUT_OLMO'] = zsl_config.ZSL_DIR_OUT_OLMO.as_posix()
    os.environ['ZSL_DIR_DATA_OLMO'] = zsl_config.ZSL_DIR_DATA_OLMO.as_posix()
    ...
    # ZSL - defaulte FileStore can hang on multi-node mila cluster. 
    #       recommendation is to use TPCStore instead
    if os.environ.get("SLURM_PROC_ID"):
        MASTER_ADDR = os.environ['MASTER_ADDR']
        MASTER_PORT = os.environ['MASTER_PORT']
        dist_kwargs["init_method"] = f"tcp://{MASTER_ADDR}:{MASTER_PORT}" 
    dist.init_process_group(**dist_kwargs)
    log.info("Process group initialized")
    log.info(dist_kwargs)
