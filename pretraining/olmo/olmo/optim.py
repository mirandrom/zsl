...
@dataclass
class TrapezoidalScheduler(Scheduler):
    warmup_steps: int
    cooldown_start: int
    alpha_f: float = 0.0
    t_max: Optional[int] = None
    ...
    def get_lr(self, initial_lr: float, step: int, max_steps: int) -> float:
        ...
        if step >= max_steps:
            ...
            # TODO: add (1-sqrt) which slightly outperforms linear
            step = step - self.cooldown_start
            cooldown_steps = max_steps - self.cooldown_start
            return initial_lr - (step * (initial_lr - eta_min) / cooldown_steps)
...
def build_scheduler(cfg: TrainConfig, sched_cfg: Optional[SchedulerConfig] = None) -> Scheduler:
    ...
    if sched_cfg.name == SchedulerType.cosine_with_warmup:
    ...
    elif sched_cfg.name == SchedulerType.trapezoidal: # ZSL
        assert sched_cfg.t_cooldown is not None
        return TrapezoidalScheduler(
            ...
            warmup_steps=int(sched_cfg.t_warmup),
            alpha_f=sched_cfg.alpha_f,
            cooldown_start=int(sched_cfg.t_cooldown),
            t_max=None if sched_cfg.t_max is None else int(sched_cfg.t_max),
            warmup_min_lr=sched_cfg.warmup_min_lr,
        )
    else:
