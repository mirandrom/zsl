...
def save_state_dict(
    ...
    *,
    ...
    if synchronize:
        barrier()
    print(f"<<!!!>>[rank:{get_global_rank()}:{get_local_rank()}.{get_fs_local_rank()}]\n path:{target_path.parent.exists()} dir:{target_path.exists()}")
...
class Checkpointer(metaclass=ABCMeta):
    ...
    def _temporary_wd(self, dir: PathOrStr) -> Generator[Path, None, None]:
        ...
        print(f"<<1>>[rank:{get_global_rank()}:{get_local_rank()}.{get_fs_local_rank()}]\t tmpdir exits: {checkpoint_dir_tmp.exists()}")
        if get_fs_local_rank() == 0:
            shutil.rmtree(checkpoint_dir_tmp, ignore_errors=True)
            checkpoint_dir_tmp.mkdir(exist_ok=True, parents=True)
            print(f"<<2>>[rank:{get_global_rank()}:{get_local_rank()}.{get_fs_local_rank()}]\t tmpdir exits: {checkpoint_dir_tmp.exists()}")
        ...
        print(f"<<3>>[rank:{get_global_rank()}:{get_local_rank()}.{get_fs_local_rank()}]\t tmpdir exits: {checkpoint_dir_tmp.exists()}")
        barrier()
        print(f"<<4>>[rank:{get_global_rank()}:{get_local_rank()}.{get_fs_local_rank()}]\t tmpdir exits: {checkpoint_dir_tmp.exists()}")
        ...
        if get_fs_local_rank() == 0:
            print(f"<<___>>[rank:{get_global_rank()}:{get_local_rank()}.{get_fs_local_rank()}]\t replacing")
        ...
        *,
        ...
        with self._temporary_wd(dir) as checkpoint_dir:
            with FSDP.state_dict_type(
                ...
                print(f"<<0>>[rank:{get_global_rank()}:{get_local_rank()}.{get_fs_local_rank()}]\t checkpoint_dir: {checkpoint_dir} \t cfg.save_overwrite: {self.cfg.save_overwrite}")
                save_state_dict(
                    checkpoint_dir,
