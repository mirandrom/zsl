...
class IterableDataset(torch.utils.data.IterableDataset[Dict[str, Any]]):
    ...
    def __init__(
        ...
        *,
        ...
        global_indices_file: Optional[Path] = None
    ):
        ...
        self.global_indices_file = global_indices_file
        self.work_dir = work_dir

        if work_dir is not None and global_indices_file is None:
            # TODO: figure out if rebuilding on training resume is actually necessary
            self.global_indices_file = Path(self.work_dir) / "global_indices.npy"
            if self.global_indices_file.exists():
                log.warning("SKIPPING BUILD_AND_SAVE_GLOBAL_INDICES")
            else:
