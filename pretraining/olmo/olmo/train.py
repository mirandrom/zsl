...
class Trainer:
    ...
    def _save_checkpoint(
        ...
        if self.indices_file is not None and get_fs_local_rank() == 0:
    ...
    # ZSL (start)
    def set_slurm_preemption_handler(self):
        self.slurm_preempting_flag = False
        def handler(signum, frame):
            log.info(f"SLURM PREEMPTION: handling signal {signum}")
            self.slurm_preempting_flag = True

        signal.signal(signal.SIGUSR1, handler)
        signal.signal(signal.SIGTERM, handler)
        signal.signal(signal.SIGINT, handler)

    def check_slurm_preempting(self) -> bool:
        slurm_preempting_flag = getattr(self, 'slurm_preempting_flag', None)
        if slurm_preempting_flag is None:
            log.info("SLURM PREEMPTION: setting handler")
            self.set_slurm_preemption_handler()
        return self.slurm_preempting_flag
    # ZSL (end)
    ...
    def fit(self):
        ...
        self.set_slurm_preemption_handler() # ZSL
        ...
        start_step = self.global_step # ZSL
        ...
        with torch_profiler as p:
            for epoch in range(self.epoch or 0, self.max_epochs):
                for batch in self.train_loader:
                    ...
                    # ZSL (start)
                    if not cancel_initiated:
                        should_cancel = False
                        if get_global_rank() == 0 and self.check_slurm_preempting():
                            log.info(f"SLURM PREEMPTION: caught {self.slurm_preempting_flag}")
                            should_cancel = True
                        cancel_initiated = synchronize_flag(should_cancel, self.device)
                        if cancel_initiated:
                            stop_at = min(stop_at, self.global_step)
                    # ZSL (end)
                    ...
                    if (
                        ...
                        if self.cfg.save_interval_unsharded_log2:
                            should_ckpt = math.log2(max(self.global_step, 1)) % 1 == 0
                        elif self.cfg.save_interval_unsharded is not None:
                            should_ckpt = self.global_step % self.cfg.save_interval_unsharded == 0
                        else:
                            should_ckpt = False
                        if should_ckpt:
                    ...
                    if python_profiler is not None:
                        if self.global_step == start_step+5: # ZSL
                            python_profiler.enable()
                        elif self.global_step == start_step+8: # ZSL
